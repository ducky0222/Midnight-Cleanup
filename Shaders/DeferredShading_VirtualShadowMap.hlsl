...
cbuffer cbPerLight
{
    VirtualDirectionalLight directionalLight;
...
Texture2D gDirectionLightShadowMap;
...
float4 PSMain(VertexOut pin) : SV_Target
{
...

    {
        ...
        if (directionalLight.isOn == 1)
        {
            ...
            currentDirectLighting = (diffuse + specular) * ndotl * directionalLight.color * directionalLight.intensity;

            currentDirectLighting = clamp(currentDirectLighting, 0.0f, 1.0f);

             // Shadow factor
            if (directionalLight.useShadow == 1)
            {
                // 월드 공간 위치를 광원 관점의 클립 공간으로 변환
                float4 shadowPos = mul(float4(positionW, 1.0f), directionalLight.viewProj);
    
                // NDC (Normalized Device Coordinate)로 변환 후 [0, 1] 텍스처 좌표로 변환
                ...
                {
                 // 그림자 맵의 깊이 값 샘플링
                    //float shadowMapDepth = gDirectionLightShadowMap.Sample(gsamPointClamp, shadowTexCoord).r;

                // 현재 픽셀의 깊이 값과 비교
                    float currentDepth = shadowPos.z/* - lightBias*/;
                    float shadowFactor = 0.0f;
                    int samples = 4; // 샘플 개수
                    float2 offset[4] = { float2(-1, -1), float2(1, -1), float2(-1, 1), float2(1, 1) };
                    for (int i = 0; i < samples; ++i)
                    {
                        float2 sampleTexCoord = shadowTexCoord + offset[i] * DLMapSize; // shadowTexelSize는 그림자 맵 텍셀 크기
                        float sampledDepth = gDirectionLightShadowMap.Sample(gsamPointClamp, sampleTexCoord).r;
                        if (currentDepth < sampledDepth)
                        {
                            shadowFactor += 0.25; // 4개의 샘플을 사용하는 경우 가중치를 0.25로 설정
                        }
                    }

                 // 그림자 판별 (0.0이면 그림자, 1.0이면 빛에 노출된 상태)
                    //float shadowFactor = currentDepth < shadowMapDepth ? 1.0f : 0.0f;

                // 기본 라이트 계산 (여기서는 단순히 그림자 적용을 위해)
    ...
    // ambient lighting
    float ambientFactor = directionalLight.intensity * 0.3f;
